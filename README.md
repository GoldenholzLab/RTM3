...
- `for_rtm3_paper.ipynb`: executed full reproduction with both manuscript images embedded.
...
The submission working copies are in `submission/epileptic_disorders/`. DOCX files preserve live Zotero citations. Final citation refreshes, saves, and page checks require Microsoft Word and Zotero; the code does not replace that step. The document-revision script is an archival transformation from the preserved September source, not part of the public simulation workflow.
